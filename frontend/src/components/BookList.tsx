...
const BookList = ({ selectedCategories }: { selectedCategories: string[] }) => {
  ...
  const handleAddToCart = async (book: Book) => {
    ...
    try {
      addToCart(book);
      setLastViewedPage(`/books?page=${currentPage}&sort=${sortDescending ? "desc" : "asc"}&categories=${selectedCategories.join(",")}`);
  ...
  const showToast = (bookTitle: string) => {
    if (toastRef.current) {
      toastRef.current.querySelector(".toast-body")!.textContent = `"${bookTitle}" added to cart!`;
  ...
  return (
    <div className="container mt-4">
      ...
      {loading ? (
        ...
        <p className="text-center text-danger">No books available for this page.</p>
      ) : (
        ...
        <ul className="pagination justify-content-center">
          {Array.from({ length: Math.max(1, Math.ceil(books.length / booksPerPage)) }, (_, i) => (
            <li key={i} className={`page-item ${currentPage === i + 1 ? "active" : ""}`}>
              <button onClick={() => paginate(i + 1)} className="page-link border-0 shadow-sm">
                {i + 1}
              </button>
            </li>
          ))}
